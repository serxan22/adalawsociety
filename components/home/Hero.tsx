...
export function Hero() {
  ...
  return (
    <section
      ref={ref}
      className="relative overflow-hidden bg-[radial-gradient(circle_at_12%_8%,rgba(63,96,118,0.08),transparent_30rem),radial-gradient(circle_at_86%_18%,rgba(174,72,94,0.08),transparent_26rem),linear-gradient(180deg,#ffffff_0%,#f7f9fc_100%)] text-als-blue"
    >
      <div className="absolute inset-0 hero-grid opacity-[0.035]" />
      <motion.div
        style={{ y }}
        className="absolute right-0 top-24 hidden h-72 w-72 rotate-45 border border-als-blue/10 lg:block"
      />
      ...
      <div className="container-wide relative grid min-h-[calc(100vh-5rem)] items-center gap-12 py-16 lg:grid-cols-[1.04fr_0.96fr]">
        <motion.div
          ...
          className="max-w-3xl"
        >
          <Badge variant="navy">{t.home.eyebrow}</Badge>
          <h1 className="mt-6 text-5xl font-bold leading-[1.05] md:text-7xl">
            {t.home.title}
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-als-muted md:text-xl">
            {t.home.subtitle}
          </p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            ...
            <MagneticButton href="/team/2025-2026" variant="ghost">
              {t.home.meetTeam}
            </MagneticButton>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="relative"
        >
          <div className="relative overflow-hidden rounded-lg border border-als-line bg-white/90 p-5 shadow-2xl shadow-als-blue/[0.12] backdrop-blur">
            <div className="legal-pattern absolute inset-0 opacity-35" />
            <div className="relative rounded-lg border border-slate-200 bg-gradient-to-br from-white via-[#F8FAFC] to-[#EEF3F8] p-6 text-als-blue shadow-xl shadow-als-blue/10">
                ...
                {["Moot court training", "Legal debates", "Research and blog", "Academic excursions"].map(
                  (item, index) => (
                    <motion.div
                      ...
                      className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white/80 px-4 py-3 text-sm font-semibold text-als-blue shadow-sm"
                    >
