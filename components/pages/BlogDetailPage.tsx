...
export function BlogDetailPage({ article, related }: {
  ...
  return (
    <>
      <article className="bg-als-blue-dark" lang={article.originalLanguage}>
        ...
        <div className="container-wide py-10">
          ...
          <div className={`mx-auto ${article.coverImage ? "mt-10" : ""} grid max-w-5xl gap-8 rounded-2xl border border-white/70 bg-white/95 p-4 shadow-xl shadow-black/10 md:p-6 lg:grid-cols-[minmax(0,1fr)_17rem]`}>
            <div className="space-y-8">
              ...
              {article.citations.length > 0 && <section className="rounded-lg border border-als-line bg-white p-6 shadow-sm">
                ...
                <ol className="mt-5 space-y-4">
                  {article.citations.map((citation, index) => (
                    <li key={`${citation.label}-${index}`} className="text-sm leading-6 text-als-muted">
                      <span className="font-semibold text-als-blue">{index + 1}. {/^\d+[.)]?$/.test(citation.label) ? "" : citation.label}</span>
            ...
            <aside className="lg:sticky lg:top-28 lg:self-start">
              <div className="rounded-lg border border-als-line bg-white p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  <AuthorAvatar name={article.author.name} image={article.author.image} />
                  <div>
                    <p className="font-bold text-als-blue">{article.author.name}</p>
                    {article.author.role && <p className="text-sm text-als-muted">{article.author.role}</p>}
                  </div>
                </div>
                {article.author.bio && <p className="mt-4 text-sm leading-6 text-als-muted">{article.author.bio}</p>}
                {article.author.socialLinks && Object.keys(article.author.socialLinks).length > 0 && <div className="mt-3 flex flex-wrap gap-2">{Object.entries(article.author.socialLinks).map(([label,url])=><a key={label} href={url} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-als-red">{label}</a>)}</div>}
