...
type Overview={totals:{total:number;published:number;draft:number;unpublished:number};assets:{authors:number;gallery:number;teamPhotos:number};recent:CmsPost[];published:CmsPost[]};
export function CmsOverview(){
  ...
  return <div className="space-y-6"><div className="flex flex-wrap gap-3">
    ...
    <div className="grid gap-3 sm:grid-cols-3">{Object.entries(data.assets).map(([label,value])=><div key={label} className="rounded-lg border border-als-line bg-white p-4"><p className="text-xs font-semibold text-als-muted">{label==="teamPhotos"?"Team photos":label[0].toUpperCase()+label.slice(1)}</p><p className="mt-1 text-2xl font-bold">{value}</p></div>)}</div>
